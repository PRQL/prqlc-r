...
---
    ...
    Output
      ...
      -- Generated by PRQL compiler version:0.13.5 target:sql.duckdb (https://prql-lang.org)
...
# Syntax error query=from a | select {{{b
    ...
    Condition
      ...
      ! Error:
       ...
       1 │ from a | select {{{b
         │ ──────────┬─────────
         │           ╰─────────── Expected one of array, doc comment, interpolated string, new line, something else or tuple, but didn't find anything before the end.
      ───╯
      Error:
         ╭─[ :1:1 ]
         │
