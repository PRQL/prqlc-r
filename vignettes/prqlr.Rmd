...
Since SQLite is used via `{RSQLite}`,
so the [dialect](https://prql-lang.org/book/queries/header.html) option is set to `"sqlite"`.
...
Available dialect names can be found with the `available_dialects()` function.

## Work with R data.frame

Using it with the `{tidyquery}` package, we can execute PRQL queries against R `?data.frame`.

Let's run a query that aggregates a data.frames `flights`,
